    ...
    def _extract_features(self, crops: List[np.ndarray]) -> np.ndarray:
        ...
        for crop in crops:
            ...
            if crop.ndim != 3 or crop.shape[2] != 3:
                logging.debug("Skipping crop with unexpected shape: %s", getattr(crop, "shape", None))
                continue
            h, w = crop.shape[:2]
            if h < 2 or w < 2:
                logging.debug("Skipping crop that is too small: %s", crop.shape)
                continue
        ...
        while start < len(tensors):
            remaining = len(tensors) - start
            current_bs = min(effective_bs, remaining)
            while True:
                try:
                    batch = torch.stack(tensors[start : start + current_bs]).to(self.device)
                    with torch.inference_mode():
                        if use_amp:
                            ...
                            feats = self.model(batch)
                    batches.append(feats.detach().cpu().numpy())
                    del batch
                    if use_amp:
...
class SiglipTeamClassifier(BaseTeamClassifier):
    ...
    def _init_siglip_backend(self, config: Dict[str, Any]) -> None:
        ...
        vision_config = SiglipVisionConfig.from_pretrained(config["model_name"])
        self._siglip_model = SiglipVisionModel.from_pretrained(
            config["model_name"], config=vision_config
        ).to(self.device)
        ...
        if color_space not in {"rgb", "hsv", "h"}:
            raise ValueError("color_space must be one of {'rgb', 'hsv', 'h'}")
        ...
        hist_channels = 1 if color_space == "h" else 3
        self._color_hist_range = (0.0, 180.0) if color_space == "h" else (0.0, 256.0)
        self._color_feature_dim = (
            hist_channels * self._color_hist_bins if self._color_hist_bins > 0 else 0
        )
    ...
    def _run_siglip_inference(self, images_rgb: List[np.ndarray]) -> np.ndarray:
        ...
        effective_bs = int(max(1, self._siglip_batch_size))
        start = 0
    ...
    def _compute_color_hist(self, image: np.ndarray) -> np.ndarray:
        ...
        hist_range = [float(self._color_hist_range[0]), float(self._color_hist_range[1])]
        for channel in cv2.split(image):
            hist = cv2.calcHist([channel], [0], None, [self._color_hist_bins], hist_range)
