...
INFERENCE_TIMING_WARMUP_FRAMES = 5
...
class _FallbackBoxTracker:
    """Greedy IoU-based tracker used when external tracking is disabled."""
    ...
    def update(self, detections: sv.Detections) -> sv.Detections:
        ...
        if track_ids:
            track_boxes = np.stack([self._tracks[tid]["box"] for tid in track_ids], axis=0)
            ious = sv.box_iou_batch(track_boxes, boxes)
            if ious.size:
                candidates: List[tuple[float, int, int]] = []
                for t_idx, tid in enumerate(track_ids):
                    for d_idx in range(len(boxes)):
                        iou_val = ious[t_idx, d_idx]
                        if iou_val >= self.iou_threshold:
                            candidates.append((float(iou_val), t_idx, d_idx))
                ...
                for iou_val, t_idx, d_idx in candidates:
                    ...
                    used_tracks.add(tid)
                    used_dets.add(d_idx)
                    self._tracks[tid]["box"] = boxes[d_idx].copy()
        ...
        for det_idx in range(len(boxes)):
            ...
            if tid is None:
                tid = self._next_id
                self._next_id += 1
                self._tracks[tid] = {"box": boxes[det_idx].copy(), "age": 0}
            else:
                self._tracks[tid]["box"] = boxes[det_idx].copy()
                self._tracks[tid]["age"] = 0
            current_ids.add(int(tid))
            tracker_ids.append(int(tid))

        self._age_tracks(current_ids)
    ...
    try:
        ...
        try:
            if config.detection.model_type == DetectionModelType.YOLO:
                ...
                from tactifoot_vision.detection.rfdetr_seg_handler import (
                    RFDETRSegHandler,
                )
        ...
        match backend:
            ...
            case "botsort":
                reid_model = str((project_root / "yolo11n.pt").resolve())
                args = BoTSORTArgs(with_reid=True, model=reid_model)
                botsort_tracker = BoTSORTTracker(args, frame_rate=int(config.tracking.frame_rate or 30))
                logger.success("BoT-SORT Tracker initialized (with ReID).")
            case other:
        ...
        min_fit_samples = 2
        fallback_tracker: Optional[_FallbackBoxTracker] = None
        ...
        if (
            not config.tracking.enabled
            and team_classifier
            and team_clf_cfg
            and team_clf_cfg.enabled
        ):
            fallback_tracker = _FallbackBoxTracker(iou_threshold=0.3, max_age=15)
        ...
        writer = None
        save_output = getattr(config.processing, "save_output_video", True)
        if save_output:
        ...
        detection_time_total_s = 0.0
        detection_time_frames = 0

        with tqdm(total=total_frames, desc="Processing frames", unit="frame") as pbar:
            for frame in frame_generator:
                do_time = frame_count >= INFERENCE_TIMING_WARMUP_FRAMES
                if do_time and _torch_cuda_available and torch is not None:  # type: ignore
                    torch.cuda.synchronize()  # type: ignore
                t0 = time.perf_counter() if do_time else None
                detections = detection_handler.detect(frame)
                if do_time:
                    if _torch_cuda_available and torch is not None:  # type: ignore
                        torch.cuda.synchronize()  # type: ignore
                    detection_time_total_s += time.perf_counter() - (t0 or 0.0)
                    detection_time_frames += 1
                if (
                ...
                if not config.tracking.enabled:
                    tracked_detections = clone_detections(other_detections)
                    if fallback_tracker is not None:
                        tracked_detections = fallback_tracker.update(tracked_detections)
                else:
                    match backend:
                        case "sam2":
                            if frame_count == 0:
                        ...
                        case "botsort":
                            tracked_detections = botsort_tracker.update(other_detections, frame)
                        case _:
        ...
        timing_path = (
            output_video_path.parent / f"{output_video_path.stem}_timing.json"
        )
        try:
            avg_ms = (
                (detection_time_total_s / detection_time_frames) * 1000.0
                if detection_time_frames > 0
                else None
            )
            fps = (
                (detection_time_frames / detection_time_total_s)
                if detection_time_total_s > 0.0
                else None
            )
            timing_payload = {
                "model_type": config.detection.model_type.value
                if config.detection is not None
                else None,
                "checkpoint_path": str(config.detection.checkpoint_path)
                if config.detection is not None and config.detection.checkpoint_path
                else None,
                "frames_total": int(frame_count),
                "timed_frames": int(detection_time_frames),
                "warmup_frames_excluded": int(INFERENCE_TIMING_WARMUP_FRAMES),
                "detection_time_total_s": float(detection_time_total_s),
                "detection_time_avg_ms": float(avg_ms) if avg_ms is not None else None,
                "detection_fps": float(fps) if fps is not None else None,
                "wall_time_total_s": float(wall_total_s),
                "output_pipeline_csv": str(export_file_path),
            }
            ...
            logger.info(f"Inference timing saved to: {timing_path}")
        except Exception as timing_exc:  # pragma: no cover
            logger.warning(
                f"Failed to write inference timing file: {timing_exc}", exc_info=True
            )
