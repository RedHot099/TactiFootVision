...
class DetectionConfig(BaseModel):
    ...
    per_class_confidence_thresholds: Optional[Dict[str, float]] = None
    ...
    @field_validator("per_class_confidence_thresholds", mode="before")
    @classmethod
    def _normalize_per_class_thresholds(cls, v: Any) -> Optional[Dict[str, float]]:
        if v is None:
            return None
        if isinstance(v, dict):
            ...
            for key, value in v.items():
                if not isinstance(key, str):
                    raise TypeError("per_class_confidence_thresholds keys must be strings")
                try:
                    cleaned[key] = float(value)
                except (TypeError, ValueError) as exc:
                    raise TypeError(
                        "per_class_confidence_thresholds values must be numeric"
                    ) from exc
            return cleaned or None
        raise TypeError("per_class_confidence_thresholds must be a dict or None")


class TrackingConfig(BaseModel):
    enabled: bool = True
    backend: Literal["bytetrack", "botsort", "sam2"] = "bytetrack"
...
class SAM2Config(BaseModel):
    ...
    max_side: Optional[int] = Field(None, ge=128)
    ...
    reseed_skip_iou_threshold: float = Field(0.0, ge=0.0, le=1.0)
    reseed_mode: Literal["add_new", "reanchor"] = "add_new"
    drop_after: int = Field(0, ge=0)
    unmatched_drop_after: int = Field(0, ge=0)
    mask_threshold: float = 0.0
    mask_open: int = Field(0, ge=0)
    mask_close: int = Field(0, ge=0)
    output_box_mode: Literal["mask", "detector", "detector_strict", "detector_blend"] = "mask"
    output_box_iou_threshold: float = Field(0.3, ge=0.0, le=1.0)
    output_box_blend_alpha: float = Field(0.7, ge=0.0, le=1.0)
    output_box_blend_alpha_by_class: Optional[Dict[str, float]] = None
    output_box_geom_min_area_ratio_by_class: Optional[Dict[str, float]] = None
    output_box_geom_max_area_ratio_by_class: Optional[Dict[str, float]] = None
    output_box_geom_max_aspect_ratio_by_class: Optional[Dict[str, float]] = None
    bbox_ema_alpha: float = Field(0.0, ge=0.0, le=1.0, description="EMA smoothing alpha for bboxes (0=disabled, 0.5=moderate smoothing)")
    ...
    @field_validator(
        "output_box_geom_min_area_ratio_by_class",
        "output_box_geom_max_area_ratio_by_class",
        "output_box_geom_max_aspect_ratio_by_class",
        mode="before",
    )
    @classmethod
    def _validate_geom_thresholds(cls, value: Any) -> Optional[Dict[str, float]]:
        if value is None:
            return None
        if isinstance(value, dict):
            ...
            for key, raw in value.items():
                if not isinstance(key, str):
                    raise TypeError("Geometry thresholds keys must be strings.")
                parsed[key] = float(raw)
                if parsed[key] < 0:
                    raise ValueError("Geometry thresholds values must be non-negative.")
            return parsed
        raise TypeError("Geometry thresholds must be a dict or None.")
...
class TrainingDetectionConfig(TrainingSharedSettings):
    ...
    early_stopping_min_delta: Optional[float] = Field(None, ge=0.0)
    early_stopping_use_ema: Optional[bool] = None
    ...
    @field_validator("output_dir", "save_checkpoint_path", mode="before")
    @classmethod
    def _pathify_optional(cls, v: Any) -> Optional[Path]:
        if isinstance(v, Path):
            return v
        if isinstance(v, str) and v:
            return Path(v)
        if v is None or (isinstance(v, str) and not v):
            return None
        raise TypeError("Path value must be a string, Path, or None")
